...
__.create = function(key, secret) {
    ...
    instance.defaultBatchLimit = 10;
    instance.batchLimits = {};
 ...
 * @param docs - max size of batch limit, as per getBatchLimit(configId)
 * @param configId
 * @returns {promise}
 */
//TODO: Add in document size check?
__.prototype.queueDocumentBatch = function(docs, configId) {
    if(docs.length > this.getBatchLimit(configId)) {
        return Q.reject('batch too large. maximum is ' + this.getBatchLimit(configId));
    }
 ...
 * NOTE: This returns a valid object
 * @returns {promise}
 */
__.prototype.retrieveSubscriptionDetails = function() {
    var url = 'https://api30.semantria.com/subscription.json';
    return this.execute('GET', url, null).then(
      function (result) {
        return Q.resolve(JSON.parse(result));
      }
    );
};
...
/**
 * Returns config batch limit size
 * @returns integer
 */
__.prototype.getBatchLimit = function(configId) {
    if (configId && this.batchLimits[configId]) {
        return this.batchLimits[configId];
    }
...
/**
 * Sets config batch limit size internally, based on each Semantria config type and the overall limit
 */
__.prototype.setBatchLimits = function() {
    var deferred = Q.defer();
    var me = this;
    me.retrieveConfigurations().then(function(configs) {
        me.retrieveSubscriptionDetails().then(function(details) {
            var outoResponseBatchSize = details.basic_settings.auto_response_limit;
            var outoCallbackBatchSize = details.basic_settings.callback_batch_limit;
            var outoBatchSize = details.basic_settings.batch_limit;
            configs.forEach(function(elem, i, a) {
                if (elem.auto_response) {
                    me.batchLimits[elem.config_id] = outoResponseBatchSize;
                } else if (elem.callback) {
                    me.batchLimits[elem.config_id] = outoCallbackBatchSize;
                } else {
                    me.batchLimits[elem.config_id] = outoBatchSize;
                }
                if (elem.is_primary) me.defaultBatchLimit = me.batchLimits[elem.config_id];
            });
            deferred.resolve();
        }, function() {
            deferred.reject('unable to retrieve subscription details');
        });
    }, function() {deferred.reject('failed to retrieve configs');});
