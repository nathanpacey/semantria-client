...
__.prototype.setBatchLimits = function() {
    ...
    this.retrieveConfigurations().then(function(configs) {
        this.retrieveSubscriptinDetails(function(details) {
            ...
            configs.forEach(function(elem, i, a) {
                if (elem.auto_response) {
                    this.batchLimits[elem.config_id] = outoResponseBatchSize;
                } else if (elem.callback) {
                    this.batchLimits[elem.config_id] = outoCallbackBatchSize;
                } else {
                    this.batchLimits[elem.config_id] = outoBatchSize;
                }
                if (elem.is_primary) this.defaultBatchLimit = this.batchLimits[elem.config_id];
            });
            deferred.resolve();
        }, deferred.reject);
    }, deferred.reject);
